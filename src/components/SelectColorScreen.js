...
const SelectColorScreen = () => {
    ...
    const changeBlue = () => {
        setIsHidden(true);
        setParam(colours.blue.image)
        setSelected(colours.blue)
    }
    const changeSky = () => {
        setIsHidden(true);
        setParam(colours.sky.image)
        setSelected(colours.sky)
    }
    const changeRed = () => {
        setParam(colours.red.image)
        setIsHidden(true);
        setSelected(colours.red)
    }
    const changeBlack = () => {
        setParam(colours.black.image)
        setIsHidden(true);
        setSelected(colours.black)
    }
    const order = () => {
        console.log(param)
        navigation.navigate('OrderScreen', { param });
    }
    ...
    return (
        <View style={{ width: '100%', height: '25%', backgroundColor: 'white' }}>
            ...
            {isHidden && (
                ...
                <TouchableOpacity onPress={order} style={{ marginLeft: '5%', backgroundColor: '#1952E2', width: '90%', height: '40px', borderRadius: '10px', marginTop: '8%' }}>
